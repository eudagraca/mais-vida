...
public class DoadoresFragment extends Fragment {
    // Components
    RecyclerView rv_posts;
    // Vars
    List<Post> postsList;
    // Utils
    PostAdapterDonnor postAdapter;
    private SharedPreferences sharedPreferences;
    FragmentManager fragmentManager;
    int fl_framelayout;

    public DoadoresFragment() {
    ...
    public View onCreateView(final LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        // Init Main
        final View view = inflater.inflate(R.layout.fragment_doadores, container, false);
...
//        });
        ...
        if (getActivity() != null){
            ...
            postAdapter = new PostAdapterDonnor(getActivity(), postsList);
            rv_posts.setAdapter(postAdapter);
        }

        // Firebase Utils
        DatabaseReference mdataRef = ConfiguracaoFirebase.getFirebase();
        mdataRef.child("Usuario").orderByChild("estado").equalTo("Doador").addValueEventListener(new ValueEventListener() {
            @Override
        ...
        postAdapter.setOnClickListener(new PostAdapterDonnor.OnItemClickListener() {
            @Override
